...
// USING createSlice from redux toolkit => advoid state muation
export const AppStateReducer1 = createSlice({
    name: "appState",
    ...
    reducers: {
            ...
            if (action.payload != null ){
                for(let key in action.payload){
                    state.data.push({
                        ...
                        completed: action.payload[key].completed,
                        editable: action.payload[key].editable,
                        key: key
                    })
                }
            }
        ...
        editTask(state,action){
            state.data = _.map(state.data, (item) => {
                if (item.key == action.payload.key){
                    return item = {...action.payload,editable : true};
                }
        ...
        saveEdit(state,action){
            state.data = state.data.map((item) => {
                if (action.payload.key == item.key){
                    item = {...action.payload,editable:false};
                }
                return item;
            });
        },
        cancelEditt(state,action){
            state.data = _.map(state.data, (item) => {
                if(item.key == action.payload.key){
                    return item = {...action.payload,editable:false};
                }
                return item;
            });
        }
